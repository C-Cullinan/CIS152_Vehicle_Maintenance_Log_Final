...
class Vehicle:
    ...
    def add_vehicle(self, make, model, year):
        ...
        if make and model and year:
            ...
            return f"All fields must contain text!"

    def add_future_maintenance_for_vehicle(self, price, part_num, description, priority):
        make = self.mainLog[0][0]
        model = self.mainLog[0][1]
        year = self.mainLog[0][2]

        futuremain = FutureMaintenance(price, part_num, description, priority)
        for vehicle in self.mainLog:
            if len(vehicle) < 4:
                vehicle.append([])
            vehicle[3].append(futuremain)
            for futmain in vehicle[3]:
                return f"{futmain.price}, {futmain.partNum}, {futmain.description}, {futmain.priority}"
    ...
    def check_viewMaint(self, c_make, c_model, c_year):
        """
        ...
        for vehicle in self.mainLog:
            print(vehicle[0])
            print(c_make)
            if vehicle[0].lower() == c_make.lower() and vehicle[1].lower() == c_model.lower() and vehicle[2] == c_year:
                # found vehicle, continue to return maintenance logs
                future_maintenance_entries = sorted(vehicle[3], key=lambda x: x.priority)  # sort priority queue
                futmain_log = []
                for futmaintenance in future_maintenance_entries:
                    futmain_log.append([f"{futmaintenance.price}, {futmaintenance.partNum}, "
                                        f"{futmaintenance.description}, {futmaintenance.priority}"])
                return futmain_log
            else:
                return f"Vehicle not found in log."
        if not self.mainLog:
            return f"Vehicle not found in log."
        ...
        if match:
            ...
            for entry in self.mainLog:
                if entry[0].lower() == c_make and entry[1].lower() == c_model and entry[2] == c_year:
                    mainT.delete('1.0', tk.END)
    ...
    def add_maintenance_for_vehicle(self, date, mileage, price, part_num, description):
        make = self.mainLog[0][0]
        model = self.mainLog[0][1]
        year = self.mainLog[0][2]

        maintence_entry = Maintenance(date, mileage, price, part_num, description)
        for vehicle in self.mainLog:
            if len(vehicle) < 4:
                vehicle.append([])
            vehicle[3].append(maintence_entry)
            for maintenance in vehicle[3]:
                return f"{maintenance.date}, {maintenance.mileage}, {maintenance.price}, {maintenance.partNum}, {maintenance.description}"
    ...
    def sort_by_price(self, make, model, year):
        ...
        for vehicle in self.mainLog:
            if vehicle[0].lower() == make.lower() and vehicle[1].lower() == model.lower() and vehicle[2] == year:
                vehicle[3].sort(key=lambda x: float(x.price))
                sorted_log = []
                for maintenance in vehicle[3]:
                    sorted_log.append([f"{maintenance.date}, {maintenance.mileage}, {maintenance.price},"
                                       f" {maintenance.partNum}, {maintenance.description}"])
                return sorted_log
    ...
    def sort_by_mile(self, make, model, year):
        # this works like a selection sort, searching for the smallest element in the array, and then putting that eleme
        # ment to the first position in the array, and so on until the entire array is sorted.
        for vehicle in self.mainLog:
            if vehicle[0].lower() == make.lower() and vehicle[1].lower() == model.lower() and vehicle[2] == year:
                vehicle[3].sort(key=lambda x: float(x.mileage))
                sorted_log = []
                for maintenance in vehicle[3]:
                    sorted_log.append([f"{maintenance.date}, {maintenance.mileage}, {maintenance.price},"
                                       f" {maintenance.partNum}, {maintenance.description}"])
        ...
        for vehicle in self.mainLog:
            if vehicle[0].lower() == make and vehicle[1].lower() == model and vehicle[2] == year:
                if len(vehicle) > 3:  # Ensure maintenance list exists
                    vehicle[3].sort(key=lambda x: float(x.mileage))  # Sort by converting price to float
        self.check_viewMaint()"""
...
class MyTestCase(unittest.TestCase):
    def test_add_vehicle_UnFilled(self):
        # Arrange
        veh = Vehicle()
        actual = veh.add_vehicle('', '', '1995')
        expected = f'All fields must contain text!'
        # Act
    ...
    def test_add_vehicle(self):
        # Arrange
        veh = Vehicle()
        actual = veh.add_vehicle('Mercedes', '300D', '1987')
        # Act
        expected = [['Mercedes', '300D', '1987']]
        # Assert
        self.assertEqual(expected, actual)

    def test_view_maintenance_bad_vehicle(self):
        # Arrange
        veh = Vehicle()
        actual = veh.check_viewMaint('', '', '')
        expected = f"Vehicle not found in log."
        # Act
        # Assert
        self.assertEqual(expected, actual)
    ...
    def test_add_maintenance(self):
        # Arrange
        veh = Vehicle()
        veh.add_vehicle('Mercedes', '300D', '1987')
        # Act
        actual = veh.add_maintenance_for_vehicle('4/26/24', '100000', '100', '1234', 'Testing')
        expected = '4/26/24, 100000, 100, 1234, Testing'
        # Assert
        self.assertEqual(expected, actual)

    def test_add_futuremain(self):
        # Arrange
        veh = Vehicle()
        veh.add_vehicle('Mercedes', '300D', '1987')
        # Act
        actual = veh.add_future_maintenance_for_vehicle('100', '1234', 'Testing', 1)
        expected = '100, 1234, Testing, 1'
        # Assert
        self.assertEqual(expected, actual)

    def test_futuremain_priority_sort(self):
        # Arrange
        veh = Vehicle()
        veh.add_vehicle('Mercedes', '300D', '1987')
        veh.add_future_maintenance_for_vehicle('100', '1234', 'Sort Test1', 3)
        veh.add_future_maintenance_for_vehicle('500', '1234', 'Sort Test2', 1)
        veh.add_future_maintenance_for_vehicle('210', '54111', 'Sort Test3', 2)
        veh.add_future_maintenance_for_vehicle('50', '55555', 'Sort Test4', 1)
        # Act
        actual = veh.check_viewMaint('Mercedes', '300D', '1987')
        expected = [['500, 1234, Sort Test2, 1'], ['50, 55555, Sort Test4, 1'], ['210, 54111, Sort Test3, 2'],
                    ['100, 1234, Sort Test1, 3']]
        # Assert
        self.assertEqual(expected, actual)
    ...
    def test_sort_by_mileage(self):
        ...
        veh = Vehicle()
        veh.add_vehicle('Mercedes', '300D', '1987')
        veh.add_maintenance_for_vehicle('4/26/2024', '125000', '500', '1234', 'Test1')
        veh.add_maintenance_for_vehicle('4/27/2024', '50000', '250', '1255', 'Test2')
        veh.add_maintenance_for_vehicle('4/28/2024', '100000', '100', '1522', 'Test3')
        # Act
        actual = veh.sort_by_mile('Mercedes', '300D', '1987')
        expected = [['4/27/2024, 50000, 250, 1255, Test2'], ['4/28/2024, 100000, 100, 1522, Test3'],
                    ['4/26/2024, 125000, 500, 1234, Test1']]
        # Assert
        self.assertEqual(expected, actual)
    ...
    def test_sort_by_price(self):
        ...
        veh.add_maintenance_for_vehicle('4/27/2024', '50000', '250', '1255', 'Test2')
        # Act
        actual = veh.sort_by_price('Mercedes', '300D', '1987')
        expected = [['4/28/2024, 100000, 100, 1522, Test3'], ['4/27/2024, 50000, 250, 1255, Test2'],
                    ['4/26/2024, 125000, 500, 1234, Test1']]
        # Assert
        self.assertEqual(expected, actual)
