...
class Vehicle:
    ...
    def check_viewMaint(self):
        ...
        FutmainT.config(state='normal')
        if len(vehicle) < 4:  # appends empty array to avoid errors with out of index calling
            vehicle.append([])
        if len(vehicle) < 5: # appends empty array to avoid errors with out of index calling
            vehicle.append([])
        if match:
            ...
            # if the specified vehicle isn't located in the log, alert user
            mainT.delete('1.0', tk.END)
